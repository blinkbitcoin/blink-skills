...
- **777 tests**, 0 failing — `node:test` framework, no test library dependencies
...
Published on ClawHub as [`blink-wallet`](https://clawhub.ai/pretyflaco/skills/blink-wallet) — check the registry for the latest served version (the mirror can briefly lag GitHub releases, which are the authoritative source). The older `blink` skill is deprecated. The full skill manifest and agent instructions are in [`blink/SKILL.md`](blink/SKILL.md).
...
```bash
npm test    # 777 tests, node:test framework, zero test dependencies
```
