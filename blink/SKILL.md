...
- **Principal only:** budgets count the payment amount, not the routing fee — for the custodial commands and `spark-send` alike. Fractional-satoshi invoice amounts are charged **ceil** (a 1400-msat invoice charges 2 sats — conservative, never round-to-nearest); invoices below 1 satoshi are refused (`l402-pay`) or not budget-tracked (`pay-invoice`).
